...
#    pygame.mouse.set_visible(False)
    ...
    for fruit in fruit_list:
        if fruit.y_change == 1.0:
            ...
            if fruit.y >= 498:
                lives -= 1
        if thug.y - 20 <= fruit.y <= thug.y + 35 and thug.x - 20 <= fruit.x <= thug.x + 35:
            score += 1
    ...
    pygame.draw.rect(screen, BROWN, [0,screen_height/2,screen_width,500])
    pygame.draw.rect(screen, SKY, [0,0,screen_width,500])

    #Drawing clouds for background
    for i in range(0,screen_width,233):
        pygame.draw.ellipse(screen, WHITE, [-25+i,50,200,100])
    
    #Drawing trees for background and their shadows
    for i in range(0,screen_width,125):
        pygame.draw.rect(screen, DBROWN, [50+i,0,50,500])
        pygame.draw.rect(screen, (210,180,110), [50+i,500,50,5])
